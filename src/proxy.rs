...
/// SAFETY: it's empty?
unsafe impl Send for ProxyV1 {}

impl Parser for ProxyV1 {
    ...
    fn parse(buf: &[u8]) -> (usize, AddressSource) {
        ...
        let Ok(_) = addr_destination_raw.parse::<IpAddr>() else {
            return (header_end, AddressSource::Error(Error::InvalidFormat));
        };
        let Ok(port_source) = port_source_raw.parse::<u16>() else {
            return (header_end, AddressSource::Error(Error::InvalidFormat));
        };
        let Ok(_) = port_destination_raw.parse::<u16>() else {
            return (header_end, AddressSource::Error(Error::InvalidFormat));
        };
        (header_end, AddressSource::FromHeader(SocketAddr::from((addr_source, port_source))))
    }
}
...
pub struct ProxyListener<P> {
    listener: TcpListener,
    parser: PhantomData<P>,
}

impl<P> ProxyListener<P> {
    pub async fn new(listener: TcpListener) -> Self {
        Self { listener, parser: PhantomData }
    }
}

impl From<TcpListener> for ProxyListener<ProxyV1> {
    fn from(value: TcpListener) -> Self {
        Self { listener: value, parser: PhantomData }
    }
}

impl<P> Listener for ProxyListener<P>
where P: Parser + Send + 'static {
    type Io = TcpStream;
    type Addr = SocketAddr;

    async fn accept(&mut self) -> (Self::Io, Self::Addr) {
        let mut header_buf: Vec<u8> = Vec::with_capacity(P::MIN_LENGTH);
        loop {
            match self.listener.accept().await {
                Ok((mut stream, _)) => {
                    let Ok(_) = stream.peek(&mut header_buf[..P::MIN_LENGTH]).await else {
                        if cfg!(feature = "tracing") {
                            tracing::error!("could not read header");
                        }
                        continue;
                    };
                    let (advance_by, source) = P::parse(&header_buf);
                    // First, try advancing the stream.
                    let Ok(_) = stream.read(&mut header_buf[..advance_by]).await else {
                        if cfg!(feature = "tracing") {
                            tracing::error!("could not read from stream");
                        }
                        continue;
                    };
                    match source {
                        ...
                        AddressSource::FromStream => {
                            let Ok(peer_addr) = stream.peer_addr() else {
                                if cfg!(feature = "tracing") {
                                    tracing::error!("could not read peer address from stream");
                                }
                                continue;
                            };
                        ...
                        AddressSource::Error(e) => {
                            if cfg!(feature = "tracing") {
                                tracing::error!("parsing PROXY protocol: {e:?}");
                            }
                            continue;
                        },
                    }
                ...
                Err(_) => {
                    if cfg!(feature = "tracing") {
                        tracing::error!("error accepting socket");
                    }
                }
            }
        }
